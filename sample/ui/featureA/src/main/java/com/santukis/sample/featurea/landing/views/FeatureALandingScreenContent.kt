...
fun FeatureALandingScreenContent(
    ...
    Column(
        ...
        ShowNavigation(onActions, featureDResult)

        FeatureALandingDetails(state)

        StateInteractions(onActions)
    }
}

@Suppress("LongMethod", "MultipleEmitters")
@Composable
private fun StateInteractions(onActions: (Action) -> Unit) {
    Button(
        onClick = {
            onActions(
...
@Composable
private fun FeatureALandingDetails(state: FeatureALandingState) {
